...
constexpr size_t CONTROL_MESSAGES_INTERVAL_MS = 100;
constexpr size_t REQUEST_MESSAGES_INTERVAL_MS = 500;
...
bool check_timeout(std::chrono::steady_clock::time_point tpoint, size_t timeout) {
  auto now = std::chrono::steady_clock::now();
  auto delta = std::chrono::duration_cast<std::chrono::milliseconds>(now - tpoint).count();
//  ESP_LOGD("TCL", "Check timeout %lld", delta);
...
void tclacClimate::loop() {
  ...
  if (hasTxData()
      && check_timeout(this->lastControlTime, CONTROL_MESSAGES_INTERVAL_MS)
      && check_timeout(this->lastSendTime, REQUEST_MESSAGES_INTERVAL_MS)) {
    takeControl();
  }
}

void tclacClimate::update() {
  if (!hasTxData() && check_timeout(this->lastSendTime, REQUEST_MESSAGES_INTERVAL_MS)) {
...
void tclacClimate::control(const ClimateCall &call) {
  ...
  if (call.get_target_temperature().has_value()) {
    target_temperature_set = call.get_target_temperature().value();
  }
  this->lastControlTime = std::chrono::steady_clock::now();
}

void tclacClimate::takeControl() {
  ESP_LOGD("TCL", "TakeControl");
  dataTX[7] = 0b00000000;
  dataTX[8] = 0b00000000;
  dataTX[9] = 0b00000000;
  ...
  if ((display_status_) && (get_tx_switch_climate_mode() != climate::CLIMATE_MODE_OFF)) {
  ...
  switch (get_tx_switch_climate_mode()) {
  ...
  switch (get_tx_switch_swing_mode()) {
  ...
  dataTX[9] = get_tx_target_temperature_set();
  ...
  // dataTX[8] = 0x08;	//mute,0,turbo,health, mode(4) mode 01 heat, 02 dry, 03 cool, 07 fan, 08 auto, health(+16), 41=turbo-heat 43=turbo-cool (turbo = 0x40+ 0x01..0x08) dataTX[9] = 0x0f;	//0 -31 ;    15 - 16 0,0,0,0, temp(4) settemp 31 - x dataTX[10] = 0x00;	//0,timerindicator,swingv(3),fan(3) fan+swing modes //0=auto 1=low 2=med 3=high
...
void tclacClimate::set_beeper_state(bool state) {
  ...
  this->additional_attr_changed = true;
  this->lastControlTime = std::chrono::steady_clock::now();
}
...
ClimatePreset tclacClimate::get_tx_switch_preset() {
  if (switch_preset.has_value()) {
    preset = switch_preset.value();
  }
  return preset.value();
}
ClimateFanMode tclacClimate::get_tx_switch_fan_mode() {
  if (switch_fan_mode.has_value()) {
    fan_mode = switch_fan_mode.value();
  }
  return fan_mode.value();
}
ClimateSwingMode tclacClimate::get_tx_switch_swing_mode() {
  if (switch_swing_mode.has_value()) {
    swing_mode = switch_swing_mode.value();
  }
  return swing_mode;
}
int tclacClimate::get_tx_target_temperature_set() {
  if (target_temperature_set.has_value()) {
    target_temperature = target_temperature_set.value();
  }

  return 31 - (int) target_temperature;
}
ClimateMode tclacClimate::get_tx_switch_climate_mode() {
  if (switch_climate_mode.has_value()) {
    mode = switch_climate_mode.value();
  }
  return mode;
}
void tclacClimate::resetTxData() {
  switch_preset.reset();
  switch_fan_mode.reset();
  switch_swing_mode.reset();
  target_temperature_set.reset();
  switch_climate_mode.reset();
  additional_attr_changed = false;
}
bool tclacClimate::hasTxData() {
  return switch_preset.has_value()
         || switch_fan_mode.has_value()
         || switch_swing_mode.has_value()
         || target_temperature_set.has_value()
         || switch_climate_mode.has_value()
         || additional_attr_changed;
}
